...
class BlogService extends cds.ApplicationService {
    async init() { 
        ...
        this.after(
            ['CREATE', 'UPDATE', 'DELETE'], 'Posts', async (_, req) => {
                const { event } = req;
                const EVENTS = {
        ...
        this.after(
            ['CREATE', 'UPDATE', 'DELETE'], 'Comments', async (_, req) => {
                const { event } = req;
                const EVENTS = {
        ...
        this.on('MyPosts', async (req) => {
            const tx = cds.tx(req), {user} = req;

            return tx.run ( SELECT.from(Posts).where({ createdBy: user.id }) );
        });

        // Increment counter for reviews considered helpful
        this.on ('like', 'Comments', (req) => {
            if (!req.user)  return req.reject(400, 'You must be identified to like a comment')
            const {comment} = req.data, {user} = req
            const tx = cds.tx(req)
            return tx.run ([
        ...
        this.on ('unlike','Comments',  async (req) => {
            ...
            const tx = cds.tx(req)
            const affectedRows = await tx.run (DELETE.from (Likes) .where ({comment_ID: review,user: user.id}))
